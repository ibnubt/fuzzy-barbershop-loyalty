...
\thanks{Proposal Tugas Mata Kuliah Sistem Cerdas, Mei 2026.}
}
...
sebuah sistem cerdas berbasis fuzzy Mamdani dengan dua input (frekuensi
...
\begin{IEEEkeywords}
sistem cerdas, fuzzy logic, Mamdani, loyalitas pelanggan, barbershop, Jakarta.
\end{IEEEkeywords}
...
\textbf{Kelemahan pertama: profil yang berbeda dipukul rata.} Bayangkan
dua pelanggan dengan profil yang sangat berbeda. Yang pertama, sebut
saja Pak Budi, datang sekali sebulan ambil paket grooming Rp~350.000.
Yang kedua, Mas Andi, marketing yang harus rapi setiap minggu, datang
tiga kali sebulan untuk paket cuci dan cukur Rp~110.000 sekali datang
(total Rp~330.000). Di skema bertingkat tadi, keduanya dapat tier yang
sama 10\%. Pak Budi karena spending-nya melewati ambang Rp~250.000,
Mas Andi karena spending-nya juga melewati ambang yang sama dan
kunjungannya melewati ambang lain (tiga kali). Tetapi profil mereka
secara intuitif berbeda. Yang satu pelanggan ``jarang tapi premium'',
yang satu ``rajin dengan paket menengah''. Skema bertingkat tidak punya
cara untuk membedakan kedua pola itu karena outputnya cuma empat nilai
diskrit (0\%, 5\%, 10\%, dan 20\%), dan semua pelanggan yang melewati
ambang yang sama dapat diskon yang sama persis.
...
sebagai agregator, masih menjadi default untuk sistem cerdas berbasis
fuzzy yang membutuhkan interpretabilitas tinggi. Alternatifnya, formulasi
...
\texttt{medium}, \texttt{high}. Bentuk MF saya pakai trapezoid pada dua
ujung (low dan high) dan triangle di tengah (medium). Koordinat
anchor yang saya pakai:
\begin{itemize}
\item \texttt{low}: trapezoid $[0, 0, 100, 175]$, artinya derajat
keanggotaan penuh ($\mu=1$) pada rentang Rp~0--100k, lalu turun ke 0
di Rp~175k. Mewakili profil sekali cukur basic.
\item \texttt{medium}: triangle $[125, 225, 350]$, puncak di Rp~225k.
Mewakili premium 1$\times$ atau basic 2--3$\times$.
\item \texttt{high}: trapezoid $[275, 400, 600, 600]$, naik dari
Rp~275k dan plateau penuh dari Rp~400k ke atas. Mewakili paket
grooming, beberapa kunjungan premium, atau member tahunan.
\end{itemize}
...
\texttt{medium}, \texttt{large}. Di kode dan dashboard saya pakai
nama Bahasa Indonesia (``Tidak Ada'', ``Kecil'', ``Sedang'', ``Besar'')
yang dipetakan ke kategori yang sama. Saya cap output di 25\% sebagai
batas operasional. Margin barbershop kelas menengah berdasarkan
pengamatan informal saya berkisar 30--40\%, jadi diskon di atas 25\%
akan terlalu agresif buat sustainability bisnis.
...
Dalam inferensi Mamdani, satu input yang berada di area overlap dua MF
secara sekaligus akan men-trigger beberapa sel dengan derajat
keanggotaan masing-masing. Sebagai ilustrasi konkret, ambil input
frekuensi 4 dan spending 150k. Pada titik ini, frekuensi punya derajat
keanggotaan 0,333 di \texttt{regular} dan 0,333 di \texttt{frequent}.
Spending punya derajat 0,333 di \texttt{low} dan 0,25 di \texttt{medium}.
Empat sel firing secara bersamaan: (\texttt{regular}, \texttt{low})
dengan kekuatan 0,333; (\texttt{regular}, \texttt{medium}) dengan
kekuatan 0,25; (\texttt{frequent}, \texttt{low}) dengan kekuatan 0,333;
dan (\texttt{frequent}, \texttt{medium}) dengan kekuatan 0,25. Output
akhir adalah pusat massa terbobot dari empat sel itu, dihitung lewat
defuzzifikasi centroid pada sub-bagian berikutnya. Untuk titik tersebut,
fuzzy menghasilkan diskon sekitar 13,8\%. Inilah mekanisme yang membuat
output fuzzy halus melewati transisi antar sel, alih-alih melompat dari
satu nilai diskrit ke nilai lain seperti pada skema bertingkat.
...
diskon $[0, 25]\%$ dengan resolusi 0,1\%. Implementasi-nya pakai
\texttt{np.arange(0, 25, 0.1)} yang menghasilkan 250 titik dari 0\%
sampai 24,9\%, sudah cukup halus untuk hitung centroid presisi 0,01
poin persentase di sekitar puncak MF output.
...
Baris ketiga (Pak Budi, paket Rp~350k 1$\times$) adalah contoh
pelanggan jarang tapi spending besar yang sudah saya bahas di
Pendahuluan. Skema bertingkat menempatkannya di tier menengah 10\%.
Sistem fuzzy memberi 15\%, dari aturan \texttt{rare AND high $\to$
medium} yang firing dengan kekuatan 0,6. Angka 0,6 berasal dari
$\min(\mu_{\texttt{rare}}=1{,}0, \mu_{\texttt{high}}=0{,}6)$ pada input
(1, 350k). Karena output \texttt{medium} adalah triangle simetris yang
puncaknya di 15, clipping pada level 0,6 menghasilkan trapezoid yang
centroid-nya tetap di 15.

Baris keempat (Mas Andi, 3$\times$ Rp~110k cukur+cuci, total Rp~330k)
menunjukkan kasus yang lebih menarik. Skema bertingkat memberi 10\%.
Trigger-nya bukan tunggal: \texttt{freq}$\geq3$ sudah cukup untuk tier
10\%, dan \texttt{spend}$\geq$Rp~250k juga sudah cukup. Tapi karena
\texttt{freq}$<5$ dan \texttt{spend}$<$Rp~400k, dia tidak naik ke tier
20\%. Sistem fuzzy memberi 19,4\% melalui dua rule yang firing
sekaligus pada input (3, 330k): \texttt{regular AND high $\to$ large}
dengan kekuatan 0,44 (karena $\mu_{\texttt{high}}$ pada Rp~330k adalah
0,44) dan \texttt{regular AND medium $\to$ medium} dengan kekuatan 0,16
(karena $\mu_{\texttt{medium}}$ pada Rp~330k tinggal 0,16). Yang
dominan adalah rule yang outputnya \texttt{large} (puncak di 22\%), dan
itulah yang menggeser centroid hasil agregasi ke 19,4\%. Kemampuan
menangkap kombinasi nonlinear seperti ini, yaitu mengangkat dua sisi
medium-tinggi ke tier yang lebih tinggi, tidak dimiliki skema
bertingkat yang outputnya cuma flat per tier.
...
10,78\%, sementara skema bertingkat memberikan 7,05\%. Rata-rata selisih
absolut antar kedua metode di sample yang sama adalah 4,27 poin
persentase. Selisih 3,7 poin pada rata-rata bukan masalah operasional.
Artinya bisnis yang beralih dari skema bertingkat ke fuzzy tidak akan
mengalami penurunan margin yang berarti.
